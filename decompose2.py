...
loss_type = "l2"
img_name = 'nature'
img = np.array(cv2.imread(f"{img_name}.jpg"))
img = img/255
...
scheduler = torch.optim.lr_scheduler.StepLR(optim, step_size=3000, gamma=0.9)
...
for step in range(total_steps):
    ...
    # print("model input output", ground_truth.shape, model_input.shape, out.shape)
    # output = out[:, 3:] * out[:, :3]
    output = out
    # print("output", output.shape, ground_truth.mean(axis=-1).unsqueeze(-1).shape)
    if loss_type=="l1":
        loss_f = torch.nn.functional.smooth_l1_loss(ground_truth, output)
    elif loss_type=="l2":
        loss_f = (((ground_truth.mean(axis=-1).unsqueeze(-1) - output))**2).mean()
    ...
    if step % 1000 == 0:
        ...
        xy_ = deepcopy(xy_grid)
        xy_[:, 0] = xy_[:, 0]*(img.shape[0]-1)
        xy_[:, 1] = xy_[:, 1]*(img.shape[1]-1)
        gt = torch.Tensor(interpolator_fn(xy_.cpu().detach().numpy())).to(device)
        generated = model(xy_grid)
        fig, axes = plt.subplots(1, 3, figsize=(8, 3))
        axes[0].imshow((gt).reshape(128, 128, 3).cpu().detach().numpy())
        axes[0].set_title("gt")
        print("visualization", gt.shape, generated.shape)
        # print(generated[:, :3].min(), generated[:, :3].max())
        axes[1].imshow((gt/generated).reshape(128, 128, 3).cpu().detach().numpy())
        axes[1].set_title("albedo")
        axes[2].imshow((generated).reshape(128, 128, 1).cpu().detach().numpy(), cmap="gray")
