...
cv2.imwrite(fr"results\IIW\Naive\{img_name}.png", 255*cv2.cvtColor((z).reshape(*shape).cpu().detach().numpy(), cv2.COLOR_BGR2RGB))

z = generated[:, :3]
axes[1].imshow((z).reshape(*shape).cpu().detach().numpy())
axes[1].set_title("albedo")
cv2.imwrite(fr"results\IIW\Naive\{img_name}_reflectance.png", 255*cv2.cvtColor((z).reshape(*shape).cpu().detach().numpy(), cv2.COLOR_BGR2RGB))

z = generated[:, 3:]
axes[2].imshow((z).reshape(shape[0], shape[1], 1).cpu().detach().numpy(), cmap="gray")
axes[2].set_title("shading")
cv2.imwrite(fr"results\IIW\Naive\{img_name}_shading.png", 255*(z).reshape(shape[0], shape[1], 1).cpu().detach().numpy())
plt.savefig(fr"results\IIW\Naive\{img_name}_combined.png")
plt.tight_layout()
plt.show()
